...
import { ScatterChart, Scatter, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, ComposedChart } from 'recharts';
...
function getDefaultInflation(): { month: string; rate: number }[] {
  ...
  for (let i = 0; i < 24; i++) {
    ...
    months.push({ month: label, rate: 2.5 });
  }
  return months;
}
...
function projectCER(
  ...
  rows.push({ date: new Date(lastOfficialDate), dateStr: formatDateISO(lastOfficialDate), cer: lastOfficialCER, isOfficial: true });
  ...
  function getMonthlyInflation(d: Date): number {
    const label = d.toLocaleDateString('es-AR', { month: 'short', year: 'numeric' });
    return inflationByMonthKey[label] ?? 0.025;
  }
  ...
  while (currentDate < endDate) {
    ...
    rows.push({ date: new Date(currentDate), dateStr: formatDateISO(currentDate), cer: currentCER, isOfficial: false });
  }
...
export default function Experimental() {
  ...
  const maxCerDateKey = maxCerDate.getTime();
  ...
  }, [lastOfficialCER, lastOfficialDate, inflation, maxCerDateKey]);
              ...
              {curvePoints.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={curvePoints} margin={{ top: 10, right: 30, left: 0, bottom: 10 }}>
                    ...
                    <XAxis dataKey="duration" type="number" tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} label={{ value: 'Duration', position: 'bottom', offset: -5, style: { fontSize: 10, fill: 'hsl(var(--muted-foreground))' } }} />
                    <YAxis tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} tickFormatter={v => `${v.toFixed(1)}%`} label={{ value: 'TNA 180 Proy.', angle: -90, position: 'insideLeft', style: { fontSize: 10, fill: 'hsl(var(--muted-foreground))' } }} />
                    <Tooltip
                    ...
                    />
                    <Scatter dataKey="yield" fill="hsl(var(--accent))" r={5} />
                  </ComposedChart>
                </ResponsiveContainer>
              ) : (
